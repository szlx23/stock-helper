...
历史 K 线缓存只用于补足均线窗口，不作为实时资格依据。每轮扫描开始时会批量拉取东方财富全 A 股实时快照，校验上海时区当天的行情时间戳，并用今开、最高、最低、最新价、成交额合成截至当前的当日日线；没有有效实时快照的股票不会进入分析线程。股票列表可缓存 12 小时；需要强制更新列表时，将 `stock_list_ttl_minutes` 设为 `0`。
